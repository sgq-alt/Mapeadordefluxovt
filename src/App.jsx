...
// ==========================================
// COMPONENTES DE ÍCONES SVG NATIVOS (PROVA DE BUGS)
// ==========================================
...
export default function App() {
  const [inputText, setInputText] = useState(defaultLocalContent);
  const [parsedData, setParsedData] = useState([]);
  const [activeTab, setActiveTab] = useState("tree-flow");
  const [showEditor, setShowEditor] = useState(true);

  // Estados Gerenciamento de Fluxos
  const [flowchartTitle, setFlowchartTitle] = useState(
    "Fluxo Vickytex Principal"
  );
  const [flowchartsList, setFlowchartsList] = useState([]);
  const [activeFlowId, setActiveFlowId] = useState("vickytex-main");

  // Estados de Autenticação e Sincronização do Firebase
  const [user, setUser] = useState(null);
  const [firebaseStatus, setFirebaseStatus] = useState("disconnected"); // 'disconnected' | 'connecting' | 'connected' | 'error'
  ...
  useEffect(() => {
    ...
    const initFirebase = async () => {
      try {
        // Autentica o usuário anonimamente de acordo com as regras de segurança
  ...
  useEffect(() => {
    if (!user) {
      ...
      const localSaved = localStorage.getItem("vickytex_local_flows");
      if (savedOfflineFlows()) {
        const flows = JSON.parse(savedConfigList() || "[]");
        setFlowchartsList(flows);
        const currentActive = flows.find((f) => f.id === activeFlowId);
        if (currentActive) {
        ...
        const initialOffline = [
          { id: "vickytex-main", title: flowchartTitle, content: inputText },
        ];
        localStorage.setItem(
          "vickytex_local_flows",
  ...
  // Função Auxiliar de Leitura de Cache Offline
  const savedOfflineFlows = () =>
    !!localStorage.getItem("vickytex_local_flows");
  const savedConfigList = () => localStorage.getItem("vickytex_local_flows");

  // 3. TERCEIRA ETAPA: Salvamento de fluxo na Nuvem ou Local
  const saveFlowToCloud = async (id, title, content) => {
    if (user) {
      setSyncStatus("saving");
      try {
        const docRef = doc(
      ...
      const offlineList = JSON.parse(savedConfigList() || "[]");
      const updatedList = offlineList.map((f) =>
        f.id === id ? { ...f, title, content } : f
      );
    ...
    <div className="min-h-screen bg-[#FDFBF7] text-slate-800 font-sans flex flex-col">
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
          ...
          <div className="w-full lg:w-4/12 bg-[#F4EFE6] border-r border-[#E5DEC9] flex flex-col p-4 space-y-4 transition-all duration-300">
            ...
            {flowchartsList.length > 0 && (
              <div className="bg-white/40 p-2.5 rounded-lg border border-[#DCD3B6] space-y-1.5 max-h-[120px] overflow-y-auto">
                <span className="text-[9px] font-bold text-[#0B2545]/70 uppercase tracking-wider block">
                  Fluxos Salvos no Banco:
                </span>
                  ...
                  <div
                    ...
                    className={`flex items-center justify-between p-1.5 rounded text-xs transition-colors ${
                      flow.id === activeFlowId
                        ? "bg-[#006335]/10 font-bold text-[#006335]"
        ...
        <div className="flex-1 bg-[#FDFBF7] overflow-y-auto p-6 transition-all duration-300 flex flex-col">
          ...
          {/* TAB 2: FLUXOGRAMA VISUAL (Horizontal com conectores e quedas em cascata) */}
          {activeTab === "visual-flow" && (
            <div className="space-y-8 max-w-5xl mx-auto w-full">
              <div className="flex items-center justify-between border-b border-[#E5DEC9] pb-3">
                <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white p-5 rounded-xl border border-[#E5DEC9] shadow-sm space-y-4">
                  ...
                  <div className="space-y-3">
                    {parsedData.some((s) =>
                      s.observations.some(
                        (o) => analyzeObservation(o).type === "oportunidade"
                      )
                    ) ? (
                      parsedData.map((sector) =>
                        sector.observations.map((obs, idx) => {
